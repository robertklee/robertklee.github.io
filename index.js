...
(function heroChat() {
  ...
  async function run(streamPrompt) {
    ...
    await stream(answer, ANSWER, { base: 26, jitter: 24, lead: 260 });
    if (myToken !== runToken) return;
  ...
  async function askTopic(topic, promptText, sourceRow) {
    ...
    await wait(300);
    if (myToken !== runToken) return;
    await stream(t.answer, variant.answer, { base: 24, jitter: 22, lead: 260 });
    if (myToken !== runToken) return;
    t.modelTag.textContent = MODELS[t.modelIdx];
    finishGenerating(t);
    scrollChatToBottom();
  ...
  async function regenFollow(t, v) {
    ...
    await wait(300);
    if (myToken !== runToken) return;
    await stream(t.answer, v.answer, { base: 24, jitter: 22, lead: 260 });
    if (myToken !== runToken) return;
    t.modelTag.textContent = MODELS[t.modelIdx];
    finishGenerating(t);
    scrollChatToBottom();
